...
/* ------------------------------------------------------------ the marker
   A gate is the one object on the approach that announced a particular
   religion, so it is a plain frame now: two piers, a lintel, a threshold, and
   nothing carved on any of them. The swept beams, the bronze collars and the
   centre strut have all gone.

   It keeps the vermilion, because the lacquer is the only saturated thing in
   the scene and the page's accent is drawn from it — take the red out of the
   approach and the palette loses the note every hover state on the document
   is answering. A closed rectangle standing at the foot of the climb reads as
   a frame around what is behind it, which is a better thing for a portfolio
   to put at its entrance than a religious threshold.                      */
function buildPortal() {
  /* the weathering costs the lacquer a lot of its chroma, so the tint puts it
  ...
  const dark = new THREE.MeshStandardMaterial({ color: 0x120c0c, roughness: .42, metalness: .14 });
  const g = new THREE.Group();
  const BASE = .78, H = 8.2, SPAN = 3.55, T = .46;

  /* the two piers — square section, not turned columns */
  [-1, 1].forEach(s => {
    const pier = new THREE.Mesh(new THREE.BoxGeometry(T, H, T), lac);
    pier.position.set(s * SPAN, BASE + H / 2, 0); pier.castShadow = true; g.add(pier);
    /* a shoe at the foot, so the pier lands on the court instead of into it */
    const shoe = new THREE.Mesh(new THREE.BoxGeometry(T + .22, .34, T + .22), dark);
    shoe.position.set(s * SPAN, BASE + .17, 0); shoe.castShadow = true; g.add(shoe);
  });

  /* the lintel, flush with the outside face of each pier */
  const lintel = new THREE.Mesh(new THREE.BoxGeometry(SPAN * 2 + T, T, T), lac);
  lintel.position.set(0, BASE + H - T / 2, 0); lintel.castShadow = true; g.add(lintel);
  /* and the threshold, which is what closes the shape into a frame */
  const sill = new THREE.Mesh(new THREE.BoxGeometry(SPAN * 2 + T, T * .62, T), lac);
  sill.position.set(0, BASE + T * .31, 0); sill.castShadow = true; g.add(sill);

  /* one dark reveal inside the head of the frame — the only detail on it, and
     it is there to give the lintel a thickness the eye can read at distance */
  const reveal = new THREE.Mesh(new THREE.BoxGeometry(SPAN * 2 - T, .12, T * .5), dark);
  reveal.position.set(0, BASE + H - T - .06, .02); g.add(reveal);

  /* the frame stands at the foot of the flight, not over the court — small
     enough that the building keeps the top of the frame to itself */
  /* The lowest thing on it is the shoe under each pier, and that starts at
     BASE — so anchoring the group at y=0 left the whole frame hanging
...
function buildLantern(x, z, s, y) {
  ...
  const paneMat = new THREE.MeshBasicMaterial({ color: hdr(1.85, 1.06, .58), fog: false, toneMapped: false });
  ...
  /* A flat cap, not a pagoda roof. The pyramidal cap, its upturned brim and
     the knob-and-finial on top were the three details that made this read as
     a temple lantern rather than as a light on a path — and the light is all
     the approach actually needs from it. Two slabs instead. */
  const cap = new THREE.Mesh(new THREE.BoxGeometry(.78, .10, .78), stone);
  cap.position.y = 1.96; cap.castShadow = true; g.add(cap);
  const cap2 = new THREE.Mesh(new THREE.BoxGeometry(.58, .09, .58), stone);
  cap2.position.y = 2.05; cap2.castShadow = true; g.add(cap2);

  const glow = new THREE.Mesh(new THREE.PlaneGeometry(3.4, 3.4),
    new THREE.MeshBasicMaterial({ map: tx(texGlow('rgba(255,204,152,.86)', 'rgba(255,158,86,.24)')),
  ...
  const lt = new THREE.PointLight(0xffb277, 2.3, 10, 2);
